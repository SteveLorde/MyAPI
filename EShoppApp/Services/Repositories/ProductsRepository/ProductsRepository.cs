...
class ProductsRepository : IProductsRepository
{
    ...
    public async Task<List<ParentCategory>> GetMainCategories()
    {
    ...
    //Streamlined into one Updated Entity
    /*
    public async Task AddQuantity(string productid, int quantity)
    {
        Product product = await _db.Products.FirstAsync(p => p.Id == Guid.Parse(productid));
        product.storequantity += quantity;
